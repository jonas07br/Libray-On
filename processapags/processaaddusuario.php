...
if(isset($_POST['cadastrar'])){
    ...
    if ($row==1){
        header("location: ../telainicio.php?s=addu&p=adderro");
    }
	//preparar um SQL para executar no banco
	if ($stmt = mysqli_prepare($con, "insert into usuario(nome,email,senha,tipo) values(?,?,?,?)")) {
		//vincular valores as interrogacoes (?)
		mysqli_stmt_bind_param($stmt,'ssss',$nome,$email,$password,$type);
